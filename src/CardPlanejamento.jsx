...
function CardPlanejamento (){
    ...
    function apareceForm(){
        document.getElementsByClassName('butForm')[0].addEventListener("click", (event) => {
            // highlight the mouseover target
            let form = document.getElementsByClassName('formArea')
            form[0].style.visibility = "visible";
            form[0].style.transition = "all 0.6s";
        });
    }

    function escondeForm(){
        document.getElementsByClassName('botaoCinza')[0].addEventListener("click", (event) => {
            // highlight the mouseover target
            let form = document.getElementsByClassName('formArea')
            form[0].style.visibility = "hidden";
            form[0].style.transition = "all 0.2s";
        });
    }
